...
public class ClearedAlarms {
    ...
    private List<Alarm> convertToDBAlarms(List<AlarmEvent> alarmEvents, long clearedUserId) {
        ...
        for (AlarmEvent alarmEvent : alarmEvents) {
            ...
            Alarm alarm = new Alarm(clearedUserId, "alarm", "some_discription", alarmEvent.getRegistryDate(), alarmEvent.getSeverityChangeTime(), alarmEvent.getSeverity(), alarmEvent.getSeverityChangeTime(), new Event(), alarmEvent.getObject(), alarmEvent.getSubobject(), (AlarmSpec)alarmEvent.getSpec(), new Alarm() );
            result.add(alarm);
        }
        return result;
    }
